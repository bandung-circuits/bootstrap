...
# the installer + verify, then reports PASS/FAIL per scheme (vscode, dsh) for
# Linux and Windows. Unattended. Each scheme gets its own clean snapshot —
# both schemes create ~/ai-workspace, so they must never run in the same boot.
#
...
# Run one scheme on the Windows VM (fresh clean-base each; only when WIN_VMX set).
test_windows_scheme(){
