#!/usr/bin/env bash
# bootstrap vscode/install.sh — one-command setup of VS Code + Claude Code +
...
# (the old bare /bootstrap/install.sh URL still works — a root shim forwards here)
#
# When piped via curl|bash, this scheme's lib/*.sh + templates are fetched from
# the GitHub Pages URL (github.io is reachable in mainland China;
# raw.githubusercontent is often blocked). The shared detect.sh lives at the
# repo root lib/ (the DSH Desktop path reuses it). When run from a clone, the
...
load_libs() {
  local script_dir
  # If run from a clone: shared detect.sh at the repo root lib/, scheme libs
  # beside this script.
  script_dir="$(cd "$(dirname "${BASH_SOURCE[0]:-$0}")" 2>/dev/null && pwd 2>/dev/null || true)"
  ...
  # Piped via curl|bash: fetch scheme libs + shared detect into a temp dir.
